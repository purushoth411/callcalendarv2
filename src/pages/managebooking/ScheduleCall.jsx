...
const ScheduleCall = () => {
  ...
  const handleDateSelect = async (dateStr, dayKey, emptySlot = true) => {
    ...
    try {
      const res1 = await fetch(
        `${API_URL}/api/helpers/getBookingData`,
        {
          ...
          body: JSON.stringify({
            ...
            showAcceptedCall: (bookingDetails?.fld_sale_type === "Presales" && (bookingDetails?.consultant_role === "SENIOR" || bookingDetails?.consultant_role === "Senior")) ? "No" : "Yes",
      ...
      // If current booking is Presales and consultant role is SENIOR, enforce caps
      if (
        bookingDetails?.fld_sale_type === "Presales" &&
        (bookingDetails?.consultant_role === "SENIOR" || bookingDetails?.consultant_role === "Senior")
      ) {
        const presalesList = Array.isArray(data1?.data)
          ? data1.data.filter((item) => item?.fld_sale_type === "Presales")
          : [];
        const presalesCount = presalesList.length;
        const presalesConfirmedCount = presalesList.filter(
          (item) => item?.fld_call_confirmation_status === "Call Confirmed by Client"
        ).length;

        if (presalesCount >= 5 || presalesConfirmedCount >= 3) {
          setAvailableSlots([]);
